    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-100 p-6 rounded-2xl shadow-sm border border-gray-200">
                <form action="{{ route('relevo.update', $relevo) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6 mb-6">
                        <div>
                            ...
                            <input type="text" name="nome_relevo" id="nome_relevo"
                                ...
                                value="{{ old('nome_relevo', $relevo->nome_relevo) }}" required>
                            @error('nome_relevo')
                        ...
                        <div>
                            <label for="tipo_relevo" class="block mb-2 text-sm font-medium text-gray-900">Tipo de
                                Relevo</label>
                            <input type="text" name="tipo_relevo" id="tipo_relevo"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                                value="{{ old('tipo_relevo', $relevo->tipo_relevo) }}">
                            @error('tipo_relevo')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="descricao_relevo" id="descricao_relevo" rows="4"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">{{ old('descricao_relevo', $relevo->descricao_relevo) }}</textarea>
                        ...
                        @if ($relevo->imagem_relevo)
                            <div class="pt-4">
                                ...
                                <img src="{{ asset('storage/' . $relevo->imagem_relevo) }}"
                                    alt="Imagem do {{ $relevo->nome_relevo }}"
                                    class="mt-2 h-40 w-auto rounded-lg shadow-sm">
                            </div>
                        @endif

                        <div>
                            <label for="imagem_relevo" class="block text-sm font-medium text-gray-900">
                    ...
                    <div class="flex items-center justify-end space-x-3 border-t border-gray-200 pt-6">
                        <a href="{{ route('biomas.manageRelevo', ['bioma' => $relevo->bioma_id]) }}"
