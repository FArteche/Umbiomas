<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Gerenciando Hidrografia do Bioma: {{ $bioma->nome_bioma }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-100 p-6 rounded-2xl shadow-sm border border-gray-200">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4 md:mb-0">Crie ou altere a hidrografia</h3>
                    ...
                    <div class="flex space-x-3">
                        ...
                        <a href="{{ route('hidrografia.create', ['bioma_id' => $bioma->id_bioma]) }}"
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach ($hidrografia as $hidrografias)
                        <div
                            class="w-full h-64 flex flex-col bg-gray-100 rounded-lg shadow-md border border-gray-200 overflow-hidden">
                            <div class="flex-shrink-0 h-32">
                                <img class="h-full w-full object-cover"
                                    src="{{ $hidrografias->imagem_hidrografia ? asset('storage/' . $hidrografias->imagem_hidrografia) : 'https://upload.wikimedia.org/wikipedia/commons/b/b9/No_photo_%282067963%29_-_The_Noun_Project.svg' }}"
                                    alt="Imagem de {{ $hidrografias->nome_hidrografia }}">
                            </div>
                            <div class="flex-grow p-3 flex flex-col justify-between">
                                <div>
                                    <h4 class="font-bold text-gray-800 truncate"
                                        title="{{ $hidrografias->nome_hidrografia }}">
                                        {{ $hidrografias->nome_hidrografia }}</h4>
                                    <p class="text-sm text-gray-600 italic truncate"
                                        title="{{ $hidrografias->tipo_hidrografia }}">
                                        {{ $hidrografias->tipo_hidrografia }}</p>
                                </div>
                                <p class="text-xs text-gray-500 mt-1 truncate"
                                    title="{{ $hidrografias->descricao_hidrografia }}">
                                    {{ $hidrografias->descricao_hidrografia }}</p>
                                <div class="flex items-center justify-end space-x-2 mt-2">
                                    <a href="{{ route('hidrografia.edit', ['hidrografia' => $hidrografias, 'return_to' => url()->current()]) }}"
                                    ...
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('hidrografia.destroy', $hidrografias) }}')"
