<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Editando: {{ $area_preservacao->nome_ap }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div x-data="areapreservacaoFormComponent()" class="bg-gray-100 p-6 rounded-2xl shadow-sm border border-gray-200">
                <form action="{{ route('area_preservacao.update', $area_preservacao) }}" method="POST"
                    ...
                    <div class="space-y-6 mb-6">
                            ...
                            <select name="tipoap_id" id="tipoap_id"
                                ...
                                @foreach ($tipos as $tipo)
                                    <option value="{{ $tipo->id_tipoap }}"
                                        {{ old('tipoap_id', $area_preservacao->tipoap_id) == $tipo->id_tipoap ? 'selected' : '' }}>
                                        {{ $tipo->nome_tipoap }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="nome_ap" id="nome_ap"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                                value="{{ old('nome_ap', $area_preservacao->nome_ap) }}" required>
                            @error('nome_ap')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="descricao_ap" id="descricao_ap" rows="4"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">{{ old('descricao_ap', $area_preservacao->descricao_ap) }}</textarea>
                            @error('descricao_ap')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        @if ($area_preservacao->imagem_ap)
                            <div>
                                ...
                                <img src="{{ asset('storage/' . $area_preservacao->imagem_ap) }}"
                                    alt="Imagem de {{ $area_preservacao->nome_ap }}"
                                    class="mt-2 h-40 w-auto rounded-lg shadow-sm">
                            </div>
                        @endif
                        <div>
                            <label for="imagem_ap" class="block text-sm font-medium text-gray-900">Substituir Imagem
                                (Opcional)</label>
